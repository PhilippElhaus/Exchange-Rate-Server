                            ...
                            if (base_currency != quote_currency_)
                            {
                                ...
                                else // Add New
                                {
                                    ...
                                    if (res != 0)
                                    {
                                        ...
                                        lock (lock_history)
                                        {
                                            if (!History_Short.Any(x => x.Key == (base_currency, quote_currency_))) History_Short.Add((base_currency, quote_currency_), new List<TimeData>() { new TimeData() { Time = DateTime.Now, Rate = (double)res } });
                                            else History_Short[(base_currency, quote_currency_)].Add(new TimeData() { Time = DateTime.Now, Rate = (double)res });

                                            if (!History_Long.Any(x => x.Key == (base_currency, quote_currency_))) History_Long.Add((base_currency, quote_currency_), new List<TimeData>() { new TimeData() { Time = DateTime.Now, Rate = (double)res } });
                                            else History_Long[(base_currency, quote_currency_)].Add(new TimeData() { Time = DateTime.Now, Rate = (double)res });
                                        }
                                    }
                                }
                    ...
                    else
                    {
                        ...
                        else // Add New
                        {
                            ...
                            if (res != 0)
                            {
                                ...
                                lock (lock_history)
                                {
                                    if (!History_Short.Any(x => x.Key == (base_currency, quote_currency))) History_Short.Add((base_currency, quote_currency), new List<TimeData>() { new TimeData() { Time = DateTime.Now, Rate = (double)res } });
                                    else History_Short[(base_currency, quote_currency)].Add(new TimeData() { Time = DateTime.Now, Rate = (double)res });

                                    if (!History_Long.Any(x => x.Key == (base_currency, quote_currency))) History_Long.Add((base_currency, quote_currency), new List<TimeData>() { new TimeData() { Time = DateTime.Now, Rate = (double)res } });
                                    else History_Long[(base_currency, quote_currency)].Add(new TimeData() { Time = DateTime.Now, Rate = (double)res });
                                }
                            }
                        }
                    }
